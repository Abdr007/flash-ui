    ...
    <div className="w-full max-w-[480px] mx-auto" style={{ animation: "fadeIn 200ms ease-out" }}>
      ...
      <div className="flex items-center justify-between mb-5">
        ...
        <div className="flex items-center gap-2">
          <span className="text-[11px] tracking-wider uppercase" style={{ color: accent }}>{side}</span>
        ...
        {(["market", "leverage", "collateral"] as Step[]).map((s, i) => (
          <div key={s} className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full transition-all"
              style={{
                background: step === s ? accent : i < ["market", "leverage", "collateral"].indexOf(step) ? accent : "rgba(255,255,255,0.1)",
        ...
        <div style={{ animation: "fadeIn 200ms ease-out" }}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {POPULAR_MARKETS.map((m) => {
              ...
              return (
                <button key={m} onClick={() => selectMarket(m)}
        ...
        <div style={{ animation: "fadeIn 200ms ease-out" }}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {LEVERAGE_OPTIONS.map((l) => (
              <button key={l} onClick={() => selectLeverage(l)}
                className="glass-card px-4 py-4 flex items-center justify-center cursor-pointer transition-all hover:scale-[1.03]"
                style={{ border: `1px solid rgba(255,255,255,0.06)` }}>
                <span className="text-[18px] font-bold num" style={{ color: l >= 20 ? "var(--color-accent-warn)" : "var(--color-text-primary)" }}>
                  {l}x
                </span>
              </button>
            ))}
          ...
          <div className="mt-3 flex items-center justify-center">
            <input type="number" min={1} max={100} placeholder="Custom..."
        ...
        <div style={{ animation: "fadeIn 200ms ease-out" }}>
          ...
          <div className="grid grid-cols-3 gap-2">
            {COLLATERAL_OPTIONS.map((c) => (
              <button key={c} onClick={() => selectCollateral(c)}
          ...
          {/* Custom input */}
          <div className="mt-3 flex items-center justify-center">
            <input type="number" min={10} placeholder="Custom $..."
              className="w-32 text-center text-[14px] num bg-transparent border rounded-lg px-3 py-2 text-text-primary outline-none"
              style={{ borderColor: "rgba(255,255,255,0.1)" }}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
