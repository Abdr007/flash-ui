...
// ============================================
// Flash AI — Home Screen (Premium Terminal)
// ============================================
// Centered balance hero + horizontal action pills + asset strip
// NOT circles like Galileo. NOT a card grid. Unique pill system.
...
export default function PortfolioHero({ onAction }: PortfolioHeroProps) {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-[560px] mx-auto pt-16 pb-4 px-6 relative">
      ...
      <div className="text-[52px] font-bold tracking-[-0.03em] leading-[1] num mb-2.5"
      ...
      {walletConnected && !walletDataError ? (
        ...
        <div className="text-[13px] mb-8" style={{ color: "var(--color-text-tertiary)" }}>
      ...
      {walletConnected && tokens.length > 0 && (
        <div className="w-full mb-7">
          <button onClick={toggleAssets}
            ...
            <div className="flex items-center flex-1">
              {tokens.slice(0, 5).map((t, i) => (
                <TokenIcon key={t.symbol} token={t} size={30}
                  style={{
                    marginLeft: i > 0 ? "-6px" : "0", zIndex: 10 - i,
                    border: "2px solid #0E131C", borderRadius: "50%",
                  }} />
              ))}
              <span className="text-[13px] text-text-tertiary ml-2.5 font-medium">
          ...
          <div className="no-scrollbar" style={{
            ...
            {tokens.map((t, i) => (
              <div key={t.symbol}
                className="flex items-center justify-between px-4 py-2.5 hover:bg-white/[0.015] relative"
                ...
                <div className="absolute left-0 top-1.5 bottom-1.5 rounded-r-full"
                  style={{ width: `${Math.max(2, Math.min(3, t.portfolioPct / 20))}px`, background: t.color, opacity: 0.4 }} />
      ...
      {/* ═══ ACTION PILLS — Unique to Flash (NOT circles like Galileo) ═══ */}
      <div className="w-full grid grid-cols-5 gap-2 mb-6">
        <ActionPill label="Trade" accent="#3B82F6" onClick={() => onAction("I want to trade")}
          icon={<><line x1="12" y1="19" x2="12" y2="5" /><polyline points="5 12 12 5 19 12" /></>} />
        <ActionPill label="Earn" accent="#00D26A" onClick={() => onAction("I want to earn yield")}
          icon={<path d="M12 2v20M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6" />} />
        <FafPill onClick={() => onAction("faf")} />
        <ActionPill label="Send" accent="#8B5CF6" onClick={() => onAction("I want to transfer tokens")}
          icon={<path d="M5 12h14M12 5l7 7-7 7" />} />
        <ActionPill label="Portfolio" accent="#F59E0B" onClick={() => onAction("show my portfolio")}
...
// ═══ ACTION PILL (Rounded vertical pill with icon + label) ═══
const ActionPill = memo(function ActionPill({ label, accent, icon, onClick }: {
  ...
  return (
    <button onClick={onClick}
      className="flex flex-col items-center gap-2 py-3 rounded-2xl cursor-pointer
        transition-all duration-150 hover:-translate-y-[1px] active:scale-[0.94]"
      style={{
        background: "rgba(14, 19, 28, 0.45)",
        border: "1px solid rgba(255,255,255,0.04)",
      }}
      onMouseEnter={(e) => { e.currentTarget.style.borderColor = `${accent}20`; e.currentTarget.style.background = "rgba(14, 19, 28, 0.7)"; }}
      onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.04)"; e.currentTarget.style.background = "rgba(14, 19, 28, 0.45)"; }}>
...
// ═══ FAF PILL (Center, dominant, glowing) ═══
const FafPill = memo(function FafPill({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick}
      className="relative flex flex-col items-center gap-2 py-3 rounded-2xl cursor-pointer
        transition-all duration-150 hover:-translate-y-[1px] active:scale-[0.94]"
      style={{
        background: "rgba(200,245,71,0.06)",
        border: "1px solid rgba(200,245,71,0.1)",
      }}
      onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(200,245,71,0.2)"; e.currentTarget.style.boxShadow = "0 0 20px rgba(200,245,71,0.08)"; }}
      onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(200,245,71,0.1)"; e.currentTarget.style.boxShadow = "none"; }}>
      <div className="w-10 h-10 rounded-xl flex items-center justify-center"
        style={{
          background: "linear-gradient(135deg, #C8F547, #a8d435)",
          boxShadow: "0 0 12px rgba(200,245,71,0.15)",
        }}>
        <span className="text-[13px] font-black" style={{ color: "#070A0F" }}>F</span>
      </div>
      <span className="text-[11px] font-semibold" style={{ color: "var(--color-accent-lime)" }}>FAF</span>
      {/* Subtle breathing glow */}
      <span className="absolute inset-0 rounded-2xl pointer-events-none"
        style={{ boxShadow: "inset 0 0 20px rgba(200,245,71,0.03)", animation: "fafBreathe 6s ease-in-out infinite" }} />
    </button>
  );
});
...
function TokenIcon({ token, size = 32, style }: {
  ...
  const showInitials = (!token.logo && !token.logoFallback) || (primaryFailed && !token.logoFallback) || (primaryFailed && fallbackFailed);
  ...
  return (
    ...
    <img key={activeSrc} src={activeSrc!} alt={token.symbol} width={size} height={size}
      style={{ width: size, height: size, flexShrink: 0, borderRadius: "50%", objectFit: "cover", ...style }}
      onError={() => { if (!primaryFailed) setPrimaryFailed(true); else setFallbackFailed(true); }} />
  );
}
