    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[440px] glass-card overflow-hidden" style={{ animation: "slideUp 200ms ease-out" }}>
        ...
        {safe(trade.leverage) > 10 && !isInFlight && (
          <div
            className="px-6 py-3 text-[13px] flex items-center gap-2"
            style={{
          ...
          >
            <span>⚠</span>
            <span>
              {safe(trade.leverage)}x leverage — liquidation {safe(liqDist).toFixed(1)}% from entry
              {safe(trade.collateral_usd) >= 500 ? " · large trade" : ""}
            </span>
          </div>
        )}
          ...
          <div className="flex items-center gap-3 px-6 py-3" style={{ borderTop: "1px solid rgba(255,255,255,0.04)" }}>
            {trade.take_profit_price && (
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[13px] num font-medium text-text-primary">
        ...
        {isInFlight ? (
          <div
            className="px-6 py-4 flex items-center gap-3 text-[14px] text-text-tertiary"
...
function DataRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div className="flex items-baseline justify-between">
      ...
      <span className="num font-medium" style={{ color: color ?? "var(--color-text-primary)" }}>
        {value}
      </span>
    </div>
  );
