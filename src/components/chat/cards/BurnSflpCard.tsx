...
export const BurnSflpCard = memo(function BurnSflpCard({ output }: { output: ToolOutput }) {
  ...
  async function handleBurn() {
    ...
    try {
      ...
      const result = await buildBurnSflp(conn, walletObj as never, percent, pool);
    ...
    <div className="w-full max-w-[460px] glass-card overflow-hidden">
      <div className="px-5 py-4">
        ...
        <div className="text-[13px] text-text-secondary mt-1">
          Burn {percent}% of your {sflpSymbol} tokens to receive USDC
        </div>
      </div>
      ...
      <div className="flex border-t border-border-subtle">
        ...
        >
          {status === "executing" ? "Burning..." : status === "error" ? "Retry" : `Burn ${sflpSymbol} for USDC`}
        </button>
      </div>
    </div>
  );
