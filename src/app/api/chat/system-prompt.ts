...
export function getSystemPrompt(context?: {
  ...
  const lines = [
    ...
    `- Earn (pools crypto/defi/gold/meme/wif/fart/ore/equity — NOT markets): earn_pools / earn_deposit (USDC→FLP) / earn_withdraw (FLP→USDC) / earn_positions / convert_flp_to_sflp (FLP→sFLP) / convert_sflp_to_flp (sFLP→FLP) / mint_sflp (USDC→sFLP directly) / burn_sflp (sFLP→USDC) / collect_stake_rewards (collect USDC rewards from staked sFLP).`,
