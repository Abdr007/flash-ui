...
export function getSystemPrompt(context?: {
  ...
  const parts: string[] = [
    ...
    ``,
    `## Earn (Liquidity Pools) — NOT a trading tool`,
    `- If user says "deposit", "withdraw", "earn", "pool", "FLP", "liquidity", "yield", "APY" → do NOT call build_trade`,
    `- Earn is handled by the UI's Earn page, not by you`,
    `- Simply respond: "Use the Earn page to deposit or withdraw from pools. Tap the Earn button on the home screen."`,
    `- NEVER try to open a trade for earn-related requests`,
    `- Pool names like "crypto", "defi", "gold", "meme" are EARN pools, NOT trading markets`,
    ``,
    `## Response Style`,
